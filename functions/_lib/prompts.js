// Prompt builders + WP/AI fetch helpers. Behavior-equivalent to server.js
// but using the global fetch and btoa (no node-fetch, no Buffer).

export const BANNED = ['casino','gambling','betting','wager','real money','deposit','withdrawal','odds','slots','payout','jackpot','jackpots','bonus','bonuses','wins','winnings','prize','prizes','rewards'];
export const TESTIMONIAL_NAMES = ['Ayla T', 'Remi K', 'Kian S', 'Noor J', 'Teo M', 'Mira L', 'Joss P', 'Leif R', 'Zara W', 'Suki B'];
export const BUILT_IN_TYPES = new Set(['post','page','attachment','revision','nav_menu_item','custom_css','customize_changeset','oembed_cache','user_request','wp_block','wp_template','wp_template_part','wp_global_styles','wp_navigation','wp_font_family','wp_font_face']);
...
export function sysCtx(brand, siteType) {
...
export function buildBatchPrompt(fields, brand, siteType, pageSlug = '') {
...
export function buildRepeaterPrompt(fieldKey, brand, siteType, rowCount) {
  const ctx = sysCtx(brand, siteType);
  const isSocial = siteType === 'social-en';

  if (fieldKey.includes('testimonial')) {
    const names = TESTIMONIAL_NAMES.slice(0, rowCount).join(', ');
    return `${ctx}
...
export async function wpFetch(wpUrl, auth, endpoint, method = 'GET', body) {
...
export async function fetchCPTSlugs(wpUrl, auth) {
  try {
